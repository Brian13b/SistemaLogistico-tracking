...
async def crear_dispositivo(dispositivo: DispositivoCreate, db: AsyncSession = Depends(get_db)):
...
async def obtener_dispositivo(dispositivo_id: str, db: AsyncSession = Depends(get_db)):
...
async def obtener_dispositivo_completo(dispositivo_id: str, limit_ubicaciones: int = Query(100, ge=1, le=1000, description="Límite de ubicaciones"), db: AsyncSession = Depends(get_db)):
...
async def obtener_dispositivo_por_imei(imei: str, db: AsyncSession = Depends(get_db)):
...
async def obtener_dispositivos_por_vehiculo(vehiculo_id: str, db: AsyncSession = Depends(get_db)):
...
async def actualizar_dispositivo(dispositivo_id: str, dispositivo_update: DispositivoUpdate, db: AsyncSession = Depends(get_db)):
...
async def asignar_dispositivo_a_vehiculo(dispositivo_id: str, vehiculo_id: str, db: AsyncSession = Depends(get_db)):
    dispositivo = await DispositivoService.obtener_dispositivo_por_id(db, dispositivo_id)
    if not dispositivo:
        raise HTTPException(status_code=404, detail="Dispositivo no encontrado")
...
async def desasignar_dispositivo(dispositivo_id: str, db: AsyncSession = Depends(get_db)):
    dispositivo = await DispositivoService.obtener_dispositivo_por_id(db, dispositivo_id)
    if not dispositivo:
        raise HTTPException(status_code=404, detail="Dispositivo no encontrado")
...
async def eliminar_dispositivo(dispositivo_id: str, db: AsyncSession = Depends(get_db)):
